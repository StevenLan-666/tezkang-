...
export default function ServiceRegistration({ onBack, onSubmit, title }: ServiceRegistrationProps) {
    ...
    <div className="bg-background-light dark:bg-background-dark text-text-main dark:text-white min-h-screen transition-colors duration-200 antialiased pb-24 flex flex-col">
      ...
      <header className="px-6 py-4 flex items-center relative justify-center">
        ...
        <div className="text-center">
          <h1 className="text-lg font-bold text-text-main dark:text-white">服务报名</h1>
      ...
      <main className="px-6 space-y-6 pt-2 flex-1 overflow-y-auto">
        <div className="flex items-center justify-between px-2">
          <div className="flex flex-col items-center gap-1 w-1/4">
            ...
            <span className="text-[10px] font-semibold text-primary">详情</span>
          </div>
        ...
        <form className="space-y-6" onSubmit={(e) => { e.preventDefault(); onSubmit(); }}>
          <section className="bg-surface-light dark:bg-surface-dark p-5 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-800">
            ...
            <div className="space-y-4">
              <div>
                ...
                <input className="w-full bg-background-light dark:bg-background-dark border-none rounded-xl py-3 px-4 text-sm text-text-main dark:text-white placeholder-slate-400/50 focus:ring-2 focus:ring-primary/50 transition-all outline-none" placeholder="张俊豪" type="text" />
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-semibold text-text-sub dark:text-slate-400 mb-1.5 ml-1">年龄</label>
                  <input className="w-full bg-background-light dark:bg-background-dark border-none rounded-xl py-3 px-4 text-sm text-text-main dark:text-white focus:ring-2 focus:ring-primary/50 transition-all outline-none" placeholder="8" type="number" />
                </div>
                <div>
          ...
          <section className="bg-surface-light dark:bg-surface-dark p-5 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-800">
            ...
            <div>
              ...
              <textarea className="w-full bg-background-light dark:bg-background-dark border-none rounded-xl py-3 px-4 text-sm text-text-main dark:text-white placeholder-slate-400/50 focus:ring-2 focus:ring-primary/50 transition-all resize-none outline-none" placeholder="例如：花生过敏，对噪音敏感..." rows={3}></textarea>
            </div>
          </section>
